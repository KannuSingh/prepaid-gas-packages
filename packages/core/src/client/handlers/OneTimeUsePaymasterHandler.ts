...
import { ONE_TIME_USE_POSTOP_GAS_LIMIT } from '@prepaid-gas/constants';
...
/**
 * Handler for OneTimeUsePaymaster contract
 *
 * Single-use activation flow:
 * - Always requires ZK proof for real transactions
 * - Uses nullifier tracking to prevent reuse
 */
export class OneTimeUsePaymasterHandler implements PaymasterHandler {
  ...
  constructor(config: PaymasterHandlerConfig) {
    ...
    this.joiningAmount = this.config.preset.paymasters.OneTimeUsePaymaster.joiningAmount;
    this.scope = this.config.preset.paymasters.OneTimeUsePaymaster.scope;
  ...
  getPaymasterAddress(): `0x${string}` {
    return this.config.preset?.paymasters.OneTimeUsePaymaster.address;
  }
  ...
  async getPaymasterStubData(parameters: GetPaymasterStubDataV7Parameters): Promise<GetPaymasterStubDataReturnType> {
    ...
    return {
      ...
      paymasterData,
      paymasterPostOpGasLimit: ONE_TIME_USE_POSTOP_GAS_LIMIT,
      sponsor: {
        name: 'Prepaid Gas Pool (One Time Use)',
        icon: undefined,
      },
    };
  }
