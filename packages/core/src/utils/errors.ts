...
import { type Address, type HexString, type PaymasterError, type ValidationError } from "@private-prepaid-gas/types";
...
export class ValidationError extends PrepaidGasError {
     ...
     */
    static fromValidationErrors(errors: readonly ValidationError[]): ValidationError {
        const messages = errors.map((e) => `${e.field}: ${e.message}`).join(", ");
        const codes = errors.map((e) => e.code).join(", ");

        return new ValidationError(`Validation failed: ${messages}`, `VALIDATION_FAILED: ${codes}`, { validationErrors: errors }, [
...
export class NetworkError extends PrepaidGasError {
    constructor(message: string, code: string, context?: Record<string, any>) {
        super({
            ...
            context,
            recoveryActions: ["Check network connectivity", "Verify RPC endpoint is accessible", "Retry operation after a delay"],
            retryable: true,
        });
    }
}
...
export class ContractError extends PrepaidGasError {
     ...
     */
    static fromPaymasterError(paymasterError: PaymasterError, contractAddress?: Address, context?: Record<string, any>): ContractError {
...
export class ConfigurationError extends PrepaidGasError {
    constructor(message: string, code: string, context?: Record<string, any>) {
        super({
            ...
            severity: ErrorSeverity.HIGH,
            context,
            recoveryActions: ["Check configuration parameters", "Verify contract addresses", "Ensure proper network settings"],
            retryable: false,
        });
    }
}
...
export class ProofError extends PrepaidGasError {
    constructor(message: string, code: string, context?: Record<string, any>) {
        super({
            ...
            severity: ErrorSeverity.HIGH,
            context,
            recoveryActions: ["Verify proof parameters", "Check identity commitment is in pool", "Ensure nullifier is unique"],
            retryable: false,
        });
    }
}
    ...
    private createDefaultStrategies(): readonly RecoveryStrategy[] {
        return [
            ...
            },
            {
                ...
                recover: async (error) => {
                    const retryAfter = (error.details.context?.retryAfter as number) || 5;
