import { PAYMASTER_DATA_OFFSET } from '@prepaid-gas/constants';
...
export function getMessageHash(
  ...
  const encodeWithChainAndEntryPoint = encodeAbiParameters(parseAbiParameters('bytes32, address, uint256'), [
    keccak256(encoded),
    entryPointAddress,
    chainId,
  ]);
