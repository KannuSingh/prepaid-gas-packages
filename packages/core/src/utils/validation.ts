...
export class UserOperationValidator {
    ...
    validatePaymasterData(userOp: PackedUserOperation): ValidationResult {
        ...
        if (userOp.paymasterAndData !== "0x") {
            if (!isHexString(userOp.paymasterAndData)) {
                ...
                if (dataLength === CONSTANTS.PAYMASTER_DATA_SIZE + 20) {
     ...
     */
    private validateContext(userOp: PackedUserOperation, errors: ValidationError[], warnings: ValidationError[]): void {
    ...
    private performStrictValidations(userOp: PackedUserOperation, warnings: ValidationError[]): void {
        ...
        if (userOp.callData.length > 10000) {
            // Arbitrary large calldata threshold
            warnings.push({
                field: "callData",
...
export class PaymasterDataValidator {
    ...
    validate(data: PaymasterData): DetailedValidationResult {
        ...
        // Validate config
        if (!isHexString(data.config) || data.config.length !== 66) {
            // 0x + 64 chars
            errors.push({
                field: "config",
        ...
        // Validate pool ID
        if (!isHexString(data.poolId) || data.poolId.length !== 66) {
            // 0x + 64 chars
            errors.push({
                field: "poolId",
 ...
 * @returns Validation result
 */
export function validateUserOperation(userOp: PackedUserOperation, context?: ValidationContext): DetailedValidationResult {
