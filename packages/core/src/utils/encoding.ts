  ...
  static encodeProof(proof: PoolMembershipProof): EncodingResult {
    try {
      const encoded = encodeAbiParameters(PROOF_TYPES, [
        ...
        proof.scope as Hex,
        proof.points.map((p) => BigInt(p)) as unknown as readonly [bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint],
      ]);
  ...
  static decodeProof(encoded: HexString): DecodingResult<PoolMembershipProof> {
    ...
    try {
      ...
      const proof: PoolMembershipProof = {
        ...
        points: decoded[5].map((p) => p.toString()) as unknown as readonly [string, string, string, string, string, string, string, string],
      };
  ...
  static hash(userOp: PackedUserOperation, entryPoint: Address, chainId: number): HexString {
    try {
      ...
      const hashInput = encodeAbiParameters(
        parseAbiParameters(['bytes userOp', 'address entryPoint', 'uint256 chainId']),
        [encodedUserOp.encoded as Hex, entryPoint as `0x${string}`, BigInt(chainId)]
      );
 ...
 */
export function createPaymasterDataEncoder(_config?: { validateOnEncode?: boolean }): typeof PaymasterDataEncoder {
 ...
 */
export function createUserOperationEncoder(_config?: { validateOnEncode?: boolean }): typeof UserOperationEncoder {
  // In the future, this could return a configured instance
  // For now, we just return the class
